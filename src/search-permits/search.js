...
async function fetchEnergovData(payload, base) {
  const url = `${base}/selfservice/api/energov/search/search`;
...
async function getTotalPages(payload, base) {
  try {
    const data = await fetchEnergovData(payload, base);
...
async function getAllResults(payload, status = "", outputfile, base) {
  try {
    const totalpages = await getTotalPages(payload, base);
    console.log("total pages available :", totalpages);

    for (let page = 1; page <= totalpages; page++) {
      payload.PermitCriteria.PageNumber = page; // set the pagenumber to the current page index
      const chunk = await fetchEnergovData(payload, base);
